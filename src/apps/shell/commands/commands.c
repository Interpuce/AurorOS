...
void shc_cowsay(char message[1024]) {
...
void shc_eclair(const char *trt) {
    if (streql(trt, "1")) {
        print("\n", 0x07);
...
void shc_cd(fs_node** current_dir, char* where, uint64_t current_user) {
    if (!where || where[0] == 0) {
...
void shc_chmod(fs_node* current_dir, uint64_t current_user_id, char* perm_arg, char* target_path) {
    ...
    if (perm_arg[0] >= '0' && perm_arg[0] <= '7') {
        uint16_t new_perms = octal_to_uint16(perm_arg);
...
void shc_cat(fs_node* current_dir, char* where, uint64_t current_user_id) {
    if (!where || where[0] == 0) {
        print_error("Missing operand");
        return;
    }
...
void shc_mkdir(fs_node* current_dir, const char* where, uint64_t owner) {
    if (!where || where[0] == 0) {
        print_error("Missing operand");
        return;
    }
