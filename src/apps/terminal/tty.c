...
reauth: {
    ...
    parse_users(buf, len, users, &users_count);
    ...
    if (welcome_msg_displayed == KFALSE) {
        ...
        if (find_user("liveuser", users, users_count) && AUROR_LIVEUSER_AUTOLOGIN) {
    ...
    UserEntry* user = find_user(username, users, users_count);
