...
echo -e "[\033[1m\033[31m*\033[0m] Auror-GUI will be moving away from using this build script"
...
echo -e "[\033[1m\033[36m*\033[0m] Assets -> C++ sources"

ffmpeg -i assets/background.png -vf scale=1024:768 -f rawvideo -pix_fmt bgra bg.raw

xxd -i bg.raw > src/images/bg.cpp
...
echo -e "[\033[1m\033[36m*\033[0m] Assembly sources -> objects"
for ASM_FILE in $(find "$SRC_DIR" -type f -name '*.asm'); do
    OBJ_FILE="$OBJECT_DIR/$(basename "${ASM_FILE%.asm}.o")"
