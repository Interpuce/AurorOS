...
void ahci_init() {
    ...
    if(dev.vendor == 0xFFFF) return;
    
    abars = (HBA_MEM*)(pci_read(dev, 0x24) & 0xFFFFFFF0);
    abars->ghc |= 0x80000000; // Enable AHCI
    
    uint32_t pi = abars->pi;
    for(int i=0; i<32; i++) {
        if(pi & (1 << i)) {
            HBA_PORT *port = &abars->ports[i];
            if((port->ssts & 0xF) != 3) continue; // Check device presence
            ...
            ahci_disks[num_ahci_disks].is_atapi = false; // TODO: Check if ATAPI
            num_ahci_disks++;
        }
    }
}
